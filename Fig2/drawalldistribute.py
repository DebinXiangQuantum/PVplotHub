...
def export_distribution_plots(excel_path, output_dir='Fig2/exported_plots/nationsFig2'):
    ...
    if '光照' not in df.columns:
        ...
        return
    df['光照'] = df['光照'].astype('int')
    df = df.sort_values(by='光照')
    x = df['光照'].values
    df['totalPV'] = df['集中式']+df['分布式']
        # 尝试匹配列名（处理“国家 类别”或“类别 国家”两种情况）
    col_c = next((c for c in df.columns if '集中式' in c), None)
    col_d = next((c for c in df.columns if '分布式' in c), None)

    # 创建符合比例的画布 (例如 40mm x 30mm)
    mm_to_inch = 1 / 25.4
    fig, ax = plt.subplots(figsize=(80 * mm_to_inch, 35 * mm_to_inch))

    # 绘图逻辑：平滑曲线 + 渐变填充
    for col_name, color, label in [(col_c, COLOR_CENTRAL, 'Utility-scale PV'), 
                                    (col_d, COLOR_DISTRIB, 'Distributed PV')]:
        ...
        if len(x[mask]) > 3:
            ...
            # 绘制主线
            ax.plot(x_smooth, y_smooth, color=color, linewidth=0.5, zorder=3, label=label)
            ...
            # 细节美化
            ax.legend(loc='upper left', frameon=False, fontsize=6, handlelength=1.5, bbox_to_anchor=(0.03, 0.95) )
            
    ax.set_xlabel('Solar Radiation (MJ/m²)', fontsize=6)
    ax.set_ylabel('Capacity (GW)', fontsize=6)
    ax.set_ylim(0,50)
    # 移除上方和右侧边框
    ...
    file_name = f"all_distributionshort.pdf"
...
if __name__ == "__main__":
    # 请确保路径正确
    EXCEL_FILE = r"Fig2/excel/SolarDistributedAll.xlsx"
